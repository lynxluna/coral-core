...
public class CommandHandlerTest {
  ...
  @Mock
  private AsyncHandler<UUID> mockUUIDHandler;
  ...
  public void testCommandHandlerCreation() throws Exception {
    ...
    doAnswer(new Answer<Void>() {
      ...
      public Void answer(InvocationOnMock invocationOnMock) throws Throwable {
        @SuppressWarnings("unchecked")
        AsyncHandler<UUID> handler = invocationOnMock.getArgument(0);
        handler.onSuccess(fakeUUID);
        return null;
      }
    }).when(mockGenerator).nextValue(Mockito.<AsyncHandler<UUID>>any());
    ...
    assertThat(theInfo.getEntityId()).isEqualTo(fakeUUID);
